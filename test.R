...
server <- function(input, output, session) {
  ...
  cleaned_data <- reactive({
    ...
    # 🔍 Діагностика (можеш залишити тимчасово)
    print(names(df))
    
    # 🔹 шукаємо колонку client_id
    client_col <- detect_client_id_column(df)
    
    if (is.null(client_col)) {
      stop("❌ Не знайдено колонку client_id. Перевір назву у файлі.")
    }
    
    # 🔹 створюємо стандартну колонку
    df <- df %>%
      mutate(
  ...
  observeEvent(input$file, {
    ...
    session$onFlushed(function() {
      tryCatch({
        df <- read_input_data(file_path)
        loaded_data(df)
      }, error = function(e) {
        loaded_data(NULL)
        showNotification(
          paste("Помилка завантаження:", e$message),
          type = "error",
          duration = 8
        )
      }, finally = {
